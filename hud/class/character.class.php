...
*/
	function attack($target) {
		$hit = 'miss';
		$dmg = $this->stats['baseDamage'] + $this->stats['strenght'] * 2;
		...
		switch($hit) {
			case 'miss':
				...
				break;
			case 'hit':    //  echo 'Trafiłeś<br>';
		          //  echo "Zadajesz $dmg obrazen<br>";
        $this->log("$this->name atakuje przeciwnika za $dmg <br>");
				$target->defend($dmg);
				break;
			case 'crit':
			//  echo "Trafiłeś krtytycznie<br>";
			      	//  echo "Zadajesz $dmg obrazen<br>";
        	$dmg *= 1.5;
        $this->log("$this->name trafia krtytycznie za $dmg <br>");
				$target->defend($dmg);
	...
	function defend($dmg) {
      ...
      $this->stats['hp'] -= $dmg;
			$this->log("$this->name otrzymuje $dmg obrazen i zostaje mu
      {$this->stats['hp']} punktow zycia <br>");
		}
	}
